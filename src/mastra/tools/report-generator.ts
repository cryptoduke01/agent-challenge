...
// Report Generator Tool - Creates comprehensive analysis reports
export const reportGeneratorTool = createTool({
  id: 'generate_report',
  description: 'Generates comprehensive code analysis reports with insights and recommendations.',
  inputSchema: z.object({
    analysisData: z.object({
      codeAnalysis: z.any().optional(),
      securityScan: z.any().optional(),
      performanceAnalysis: z.any().optional(),
      documentationAnalysis: z.any().optional(),
      repositoryAnalysis: z.any().optional(),
    }),
  ...
  execute: async ({ context }) => {
    try {
      const { analysisData, reportType, includeCharts } = context;
      
      // Generate comprehensive report
      const report = await generateComprehensiveReport(analysisData, reportType, includeCharts);
      ...
      return {
        report: {
          title: 'CodeGuardian Analysis Report',
          summary: `Report generation failed: ${error.message}`,
          executiveSummary: 'Unable to generate report due to analysis errors.',
          sections: [{
            title: 'Error',
            content: `Failed to generate report: ${error.message}`,
            recommendations: ['Fix analysis errors and regenerate report'],
          }],
        },
        metadata: {
          generatedAt: new Date().toISOString(),
...
async function generateComprehensiveReport(analysisData: any, reportType: string, includeCharts: boolean): Promise<{
  title: string;
  summary: string;
...
function generateMainSummary(analysisData: any): string {
  ...
  if (analysisData.documentationAnalysis) {
    summary += `## 📚 Documentation Analysis\n\n`;
    summary += `- **Coverage**: ${analysisData.documentationAnalysis.coverage?.coverage || 'N/A'}%\n`;
    summary += `- **Functions Documented**: ${analysisData.documentationAnalysis.coverage?.functions || 0}\n`;
    summary += `- **Classes Documented**: ${analysisData.documentationAnalysis.coverage?.classes || 0}\n`;
    summary += `- **APIs Documented**: ${analysisData.documentationAnalysis.coverage?.apis || 0}\n\n`;
  }
...
function generateReportSections(analysisData: any, reportType: string): Array<{
  ...
  if (analysisData.documentationAnalysis) {
    sections.push({
      ...
      content: generateDocumentationContent(analysisData.documentationAnalysis),
      metrics: {
        coverage: analysisData.documentationAnalysis.coverage?.coverage || 0,
        functions: analysisData.documentationAnalysis.coverage?.functions || 0,
        classes: analysisData.documentationAnalysis.coverage?.classes || 0,
      },
      recommendations: analysisData.documentationAnalysis.suggestions || [],
    });
  }
  
  // Repository Section
  if (analysisData.repositoryAnalysis) {
    sections.push({
      title: 'Repository Analysis',
      content: generateRepositoryContent(analysisData.repositoryAnalysis),
      metrics: {
        health: analysisData.repositoryAnalysis.analysis?.overallHealth || 0,
        files: analysisData.repositoryAnalysis.statistics?.totalFiles || 0,
        languages: Object.keys(analysisData.repositoryAnalysis.statistics?.languages || {}).length,
      },
      recommendations: analysisData.repositoryAnalysis.analysis?.recommendations || [],
    });
  }
...
function generateDocumentationContent(analysis: any): string {
  let content = `**Documentation Coverage**: ${analysis.coverage?.coverage || 0}%\n\n`;
  
  if (analysis.coverage) {
    content += `**Coverage Breakdown**:\n`;
    content += `- Functions Documented: ${analysis.coverage.functions}\n`;
    content += `- Classes Documented: ${analysis.coverage.classes}\n`;
    content += `- APIs Documented: ${analysis.coverage.apis}\n\n`;
  }
  ...
  if (analysis.documentation?.readme) {
    ...
    content += `- Code Comments: Enhanced\n\n`;
  }
  
  return content;
}

function generateRepositoryContent(analysis: any): string {
  let content = `**Repository Health**: ${analysis.analysis?.overallHealth || 0}/100\n\n`;
  
  if (analysis.repository) {
    content += `**Repository Info**:\n`;
    content += `- Name: ${analysis.repository.name}\n`;
    content += `- Branch: ${analysis.repository.branch}\n`;
    content += `- Contributors: ${analysis.repository.contributors}\n\n`;
  }
  
  if (analysis.statistics) {
    content += `**Statistics**:\n`;
    content += `- Total Files: ${analysis.statistics.totalFiles}\n`;
    content += `- Total Lines: ${analysis.statistics.totalLines}\n`;
    content += `- Languages: ${Object.keys(analysis.statistics.languages || {}).join(', ')}\n\n`;
  }
  
  if (analysis.analysis?.issues?.length > 0) {
    content += `**Repository Issues**:\n`;
    analysis.analysis.issues.slice(0, 5).forEach((issue: string) => {
      content += `- ${issue}\n`;
    });
  }
...
function generateCharts(analysisData: any): Array<{
  ...
  if (analysisData.codeAnalysis?.issues || analysisData.securityScan?.vulnerabilities) {
    const issuesData = {
      labels: ['Critical', 'High', 'Medium', 'Low'],
      datasets: [{
        data: [
          (analysisData.codeAnalysis?.issues?.filter((i: any) => i.severity === 'critical').length || 0) +
          (analysisData.securityScan?.vulnerabilities?.filter((v: any) => v.severity === 'critical').length || 0),
          (analysisData.codeAnalysis?.issues?.filter((i: any) => i.severity === 'high').length || 0) +
          (analysisData.securityScan?.vulnerabilities?.filter((v: any) => v.severity === 'high').length || 0),
          (analysisData.codeAnalysis?.issues?.filter((i: any) => i.severity === 'medium').length || 0) +
          (analysisData.securityScan?.vulnerabilities?.filter((v: any) => v.severity === 'medium').length || 0),
          (analysisData.codeAnalysis?.issues?.filter((i: any) => i.severity === 'low').length || 0) +
          (analysisData.securityScan?.vulnerabilities?.filter((v: any) => v.severity === 'low').length || 0),
        ],
